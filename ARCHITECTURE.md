...
The com0com suite consists of four main components:
...
- **Bus FDO (PDO):** A single root-enumerated device (`ROOT\COM0COM\0000`) that
  acts as the bus. It is created by the PNP manager from the INF file.
- **Port FDOs:** Child devices created by the bus driver. Each port appears as a
  ...
  - `com0com\cncport` — CNC port class (default names CNCA0, CNCB0)
  - `com0com\comport` — COM port class (auto-assigned names COM3, COM4)
...
(DTR, RTS, CTS, DSR, DCD, RI) are mirrored between paired ports. This creates
a virtual null-modem cable.

### Key Data Structures

- `C0C_COMMON_EXTENSION` — base device extension for all device types
...
Since the fixes applied in this build:

- 8 data bits, no parity, 1 stop bit, 9600 baud
- Flow control: DTR/DSR and RTS/CTS enabled
- XON/XOFF characters: 0x11 / 0x13

### Pinout Customization

The driver supports custom pinout wiring between paired ports via the
`PinMap` function. Each signal pin (CTS, DSR, DCD, RI) can be mapped to
any local or remote pin with optional inversion.
...
- `C0C_MCR_OPEN` mask (DTR + RTS) is asserted when the port opens
...
- `LoadComDbLocal()` / `SaveComDbLocal()` — read/write the local COM DB
- Reserve and release COM port numbers
- Query which COM port numbers are in use (`busynames`)

### INF File Parsing (`inffile.cpp`)

Parses the driver INF files (`com0com.inf`, `cncport.inf`, `comport.inf`)
to extract driver installation parameters.

### Driver Installation

`setupc.exe install` calls the Windows PNP manager to create port pairs:

1. Creates the root-enumerated bus device via `UpdateDriverForPlugAndPlayDevices`
...
The `TelnetProtocol` class (in `telnet.cpp`) implements:
...
This connects `\\.\CNCA0` to a TCP server at `192.168.1.1:5000`.
...
- `--echo-route=FROM:TO` — echo route (data sent back to sender)
...
1. **In-method** (`pInMethod`): called for data arriving at the port
2. **Out-method** (`pOutMethod`): called for data leaving the port
...
- `pConfigStart()` / `pConfig(pArg)` / `pConfigStop()` — parse configuration options

Note: most filter plugins set `pConfig`/`pConfigStart`/`pConfigStop` to NULL
and parse options in their `pCreate` function via `argc`/`argv`. Only the
`serial`, `tcp`, and `trace` plugins implement the config API.

### Filter-Specific Routines (`FILTER_ROUTINES_A`)

- `pCreate(hMasterFilter, hConfig, argc, argv)` — create a filter instance
...
- `pInMethod(hFilter, hFilterInstance, pInMsg, ppEchoMsg)` — process an incoming message
- `pOutMethod(hFilter, hFilterInstance, hFromPort, pOutMsg)` — process an outgoing message

### Hub Routines (`HUB_ROUTINES_A`)

The hub provides 20 callback functions to plugins for buffer allocation,
message creation/manipulation, port naming, timers, and argument parsing.

### Message Types (`hubmsg.cpp`)
...
The com0com driver and tools store configuration in the registry.

### Driver Parameters

`HKLM\SYSTEM\CurrentControlSet\Services\com0com\Parameters`

Port-specific parameters are stored per port device instance under
`HKLM\SYSTEM\CurrentControlSet\Enum\root\com0com\<instance>\Device Parameters\Port\`
...
Standard Windows uninstall registry keys: DisplayName, Publisher, Version,
UninstallString, etc.
...
The paired port's read buffer acts as the null-modem link. No data is buffered
in intermediate storage — the write to port A directly populates port B's read
buffer under a spinlock.
